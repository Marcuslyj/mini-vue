...
const createVNode = function (type, props, children) {
    const vnode = {
        ...
        props: props || {},
        children,
        shapeFlag: getShapeFlag(type), // string ? 1 : 4
    };
    if (Array.isArray(children)) {
        vnode.shapeFlag |= 16; // 16 = '10000'
        // vnode.shapeFlag = vnode.shapeFlag|16
    }
    else if (typeof children === "string") {
        vnode.shapeFlag |= 8; // 8 = '1000'
    }
    else {
        // ?
    }
...
function normalizeChildren(vnode, children) {
    if (typeof children === "object") {
        if (vnode.shapeFlag & 1); // 最后一位是1，指的是字符串？
        else {
            vnode.shapeFlag |= 32; // 32 = '100000'
        }
    }
}
...
function createGetter(isReadonly = false, shallow = false) {
    return function get(target, key, receiver) {
        ...
        const res = Reflect.get(target, key, receiver); // receiver是getter调用时候的this值
        if (!isReadonly) {
...
class RefImpl {
    constructor(value) {
        ...
        // 1. 数据响应式
        this._value = convert(value); // object value ? reactive(value) : value
        // 2. 创建依赖容器
        this.dep = createDep(); // 依赖容器, Set实例
    }
    // 3. getter
    // inst.value
    get value() {
...
export {
    computed, createApp, createAppAPI, createVNode as createElementVNode, createRenderer, createTextVNode, effect, getCurrentInstance,
    h, // createVNode
    inject, isProxy, isReactive, isReadonly, isRef, provide, proxyRefs, reactive, readonly,
    createRef  as ref,
    registerRuntimeCompiler, renderSlot, shallowReadonly, stop, toDisplayString, unRef
};
